...
public final class UnicornLevitationBudget {
    ...
    private static final int BUFFER_TICKS = 6;
    ...
    private int packetCredit = UnicornLevitationRules.MOVE_PACKET_BURST;
    private boolean horizontalGrantSpent, upGrantSpent, downGrantSpent;
    ...
    public void advance(long nextTick, float yaw, float forward, float sideways, UnicornLevitationMath.Mode mode,
        ...
        expected = UnicornLevitationMath.step(expected, yaw, forward, sideways, mode, feetY, surfaceY);
        var forecast = UnicornLevitationMath.step(expected, yaw, forward, sideways, mode, feetY + expected.y(), surfaceY);
        ...
        int grant = first ? 2 : 1;
        horizontal = Math.min(HORIZONTAL_MARGIN + BUFFER_TICKS * h, horizontal + (horizontalGrantSpent ? 0 : grant * h));
        up = Math.min(UP_MARGIN + BUFFER_TICKS * u, up + (upGrantSpent ? 0 : grant * u));
        down = Math.min(JITTER + BUFFER_TICKS * d, down + (downGrantSpent ? 0 : grant * d));
        horizontalGrantSpent = upGrantSpent = downGrantSpent = false;
    }
    ...
    public Verdict validate(long now, double nextX, double nextY, double nextZ) {
        ...
        if (packetTick == Long.MIN_VALUE) packetTick = now;
        if (now > packetTick) {
            long elapsed = Math.min(UnicornLevitationRules.MOVE_PACKET_BURST, now - packetTick);
            packetCredit = Math.min(UnicornLevitationRules.MOVE_PACKET_BURST,
                    packetCredit + (int) elapsed * UnicornLevitationRules.MAX_MOVE_PACKETS);
            packetTick = now;
        }
        if (packetCredit == 0) return Verdict.REJECT;
        packetCredit--;
        // 短时积压可以集中处理，但每个包仍消耗同一份有限位移额度。
    ...
    public void teleport(double confirmedX, double confirmedY, double confirmedZ, boolean keepX, boolean keepY, boolean keepZ) {
        reanchor(confirmedX, confirmedY, confirmedZ);
        expected = new UnicornLevitationMath.Motion(keepX ? expected.x() : 0, keepY ? expected.y() : 0, keepZ ? expected.z() : 0);
        double settleH = Math.min(forecastHorizontal, Math.max(0, -horizontal));
        double settleU = Math.min(forecastUp, Math.max(0, -up)), settleD = Math.min(forecastDown, Math.max(0, -down));
        // 已借出的预测先结算，下一刻不重复补给；绝对轴不再携带旧速度债务。
        horizontalGrantSpent |= settleH > EPSILON; upGrantSpent |= settleU > EPSILON; downGrantSpent |= settleD > EPSILON;
        horizontal = Math.min(horizontal + settleH, HORIZONTAL_MARGIN);
        up = Math.min(up + settleU, UP_MARGIN); down = Math.min(down + settleD, JITTER);
        forecastHorizontal = keepX || keepZ ? Math.max(0, forecastHorizontal - settleH) : 0;
        forecastUp = keepY ? Math.max(0, forecastUp - settleU) : 0;
        forecastDown = keepY ? Math.max(0, forecastDown - settleD) : 0;
    }
