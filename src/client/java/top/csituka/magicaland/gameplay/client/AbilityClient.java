...
public final class AbilityClient {
    ...
    public static Text name(int slot) {
        return Text.translatable(slot == 0 ? "text.magicaland_gameplay.remote.name" : "text.magicaland_gameplay.sense.name");
    }
    public static Text unavailable(int slot) { return slot == 0 ? RaceClient.abilityUnavailable() : EarthSenseClient.unavailable(); }
