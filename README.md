...
当前开发版本为 `0.3.4`。独角兽可使用[自我悬浮](docs/unicorn-levitation.md)，进行魔法托举、落地缓冲和液面悬浮；天马[飞行](docs/pegasus-flight.md)已进入测试，可悬停、滑翔和完成回环横滚；陆马可通过[震动感知](docs/earth-sense.md)察觉附近的地面活动。各系统的使用说明和验证记录见[文档目录](docs/README.md)。
...
- Magical Land: Gameplay `0.3.4`。
- Magical Land: Appearance `0.3.6`，以及其 GeckoLib 依赖（4.7 或更新版本）。

Gameplay 当前支持 Appearance `>=0.3.6 <0.4.0`。玩家安装普通模组 JAR；升级时同步更新客户端和服务器的 Gameplay，并使用配套外观包。
...
- 首次进入世界时选择种族。独角兽可使用念力出窍、自我悬浮和御物，天马可飞行，陆马可使用震动感知。
- 按住 **R** 打开能力轮盘，选中能力后松开。
...
使用 Gradle 9.4.1 wrapper、Loom 1.16.3，Java 输出目标为 17。构建工具使用外观公共 API 1.6 的开发产物进行编译，运行时加载完整外观模组。构建入口、模块职责和存档迁移说明见[开发与仓库边界](docs/repository-boundary.md)；后续工作见[待办清单](TODO.md)。
